...
namespace VESHTest.Module.reg {
    public class Action : GCL.Project.VESH.V.Control.IAction {
        ...
        public void getCard(HttpRequest request, HttpResponse response, HttpContext context, GCL.Project.VESH.V.Control.Session.SessionDataManager session)
        {
            var ret = new GCL.Db.Ni.NiTemplateManager(session.Middler, "Ni").ExcuteQuery("db2", "VESHTest.Module.reg.getCard", new System.Collections.Hashtable() { { "IDCard", request["IDCard"] } });
            if (ret.DataSet.Tables.Count > 0)
            {
                session.Status = Convert.ToString(ret.GetFirstCell(0));
            }
        }
        public void getlMail(HttpRequest request, HttpResponse response, HttpContext context, GCL.Project.VESH.V.Control.Session.SessionDataManager session)
        {
            var ret = new GCL.Db.Ni.NiTemplateManager(session.Middler, "Ni").ExcuteQuery("db2", "VESHTest.Module.reg.getlMail", new System.Collections.Hashtable() { { "Mail", request["Mail"] } });
            if (ret.DataSet.Tables.Count > 0)
            {
                session.Status = Convert.ToString(ret.GetFirstCell(0));
            }
